...
const buyDiamond = [
    ...
    {name: "Top Up Mobile Legend", price: "Rp 32.490", diamond: "113 Diamonds (102 + 11 Bonus)"},
    {name: "Top Up Mobile Legend", price: "Rp 38.100", diamond: "128 Diamonds (117 + 11 Bonus)"},
    {name: "Top Up Mobile Legend", price: "Rp 43.300", diamond: "148 Diamonds (134 + 14 Bonus)"},
    {name: "Top Up Mobile Legend", price: "Rp 54.100", diamond: "184 Diamonds (167 + 17 Bonus)"},
]
buyDiamond.forEach(buyDiamond => {
    const card = document.createElement("div")
    card.classList.add("item")
    card.innerHTML = `
        <h5>${buyDiamond.diamond}</h5>
        <br/>
...
const buyMembership = [
    {name: "Top Up Mobile Legend", price: "Rp 30.000", member: "Weekly Diamond Pass"},
    {name: "Top Up Mobile Legend", price: "Rp 59.900", member: "Weekly Diamond Pass x2"},
    {name: "Top Up Mobile Legend", price: "Rp 88.500", member: "Weekly Diamond Pass x3"},
    {name: "Top Up Mobile Legend", price: "Rp 118.000", member: "Weekly Diamond Pass x4"},
    {name: "Top Up Mobile Legend", price: "Rp 117.500", member: "Weekly Diamond Pass x5"},
    {name: "Top Up Mobile Legend", price: "Rp 158.900", member: "Twilight Pass"}
]
buyMembership.forEach(buyMembership => {
    const card = document.createElement("div")
    card.classList.add("item")
    card.innerHTML = `
        <h5>${buyMembership.member}</h5>
        </br>
...
buyButton.addEventListener("click", async () => {
    ...
    if (!selectedDiamond) {
        ...
        return;
    } else if (!userId.value && !zoneId.value) {
    ...
    console.log("Nominal terpilih:", selectedDiamond.querySelector("h5").innerText);
    ...
    const pesanan = {
        diamond: selectedDiamond.querySelector("h5").innerText,
    ...
    // Jika metode Cash, bulatkan sesuai aturan: sisa < =500 -> ke 500, sisa >500 -> ke 1000
    ...
    if (dataPesanan) {
        ...
        dataPesanan.innerHTML = `
            ...
            <p><strong>Nama Pemain:</strong> ${pesanan.playerName ? pesanan.playerName : '-'}</p>
            <p><strong>Metode:</strong> ${pesanan.paymentMethod}</p>
            <p><strong>User ID:</strong> ${pesanan.userId} (${pesanan.zoneId})</p>
            <p><strong>Kode Promo:</strong> ${pesanan.nopro}</p>
            <p><strong>Harga Diamond:</strong> ${pesanan.priceText}</p>
...
if (konfirmasiBtn && !konfirmasiBtn.dataset.listenerAttached) {
    const handleKonfirmasi = () => {
        ...
        const produk = selectedDiamond && selectedDiamond.querySelector("h5") ? selectedDiamond.querySelector("h5").innerText : "";
        ...
        const url =
            ...
            "Saya ingin top up Mobile Legend dengan nominal *" + encodeURIComponent(produk) +
            "%0Amenggunakan metode pembayaran *" + encodeURIComponent(metode) + "*."+
            "*%0AInvoice: *" + encodeURIComponent(invoiceId) +
            "*%0AUser ID: *" + encodeURIComponent(userId ? userId.value : "") + "(" + encodeURIComponent(zoneId ? zoneId.value : "") + ")" +
...
// Sederhana: tambahkan input `playerName` di bawah `userId` agar user bisa
// menuliskan/mengubah nickname pemain secara manual. Hasil disimpan ke
// localStorage dengan kunci `ml_nick_<userId>` saat input blur.
if (inputId) {
    ...
    if (!playerNameInput) {
        ...
        playerNameInput.placeholder = 'Nama Pemain (tidak harus sama persis)';
        playerNameInput.style.fontSize = '0.95em';
        playerNameInput.style.padding = '12px';
        playerNameInput.style.width = '100%';
        wrap.appendChild(playerNameInput);
        if (inputId.parentNode) inputId.parentNode.insertBefore(wrap, inputId.nextSibling);
    }

    // Saat inputId berubah/blur, coba isi dari cache jika tersedia
    inputId.addEventListener('blur', () => {
        const uid = (inputId.value || '').trim();
        if (!uid) return;
        const cached = localStorage.getItem('ml_nick_' + uid);
        if (cached) playerNameInput.value = cached;
    });

    // Simpan nama pemain ke cache saat user mengisi dan blur
    playerNameInput.addEventListener('blur', () => {
        const uid = (inputId.value || '').trim();
        const val = (playerNameInput.value || '').trim();
        if (uid && val) {
            try { localStorage.setItem('ml_nick_' + uid, val); } catch (e) {}
        }
    });

    // Jika page sudah memiliki inputId dan cache, isi otomatis
    (function initPlayerNameFromCache() {
        const uid = (inputId.value || '').trim();
        if (!uid) return;
        const cached = localStorage.getItem('ml_nick_' + uid);
        if (cached) playerNameInput.value = cached;
